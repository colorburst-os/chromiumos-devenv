...
void RuleBasedEngineConnectionFactory::ConnectToInputMethod(
  ...
  // Unlike the historical implementation, report failure when `ime_spec` is
  // not a rule-based IME, so non-rule-based engines (FST, CJK) fail cleanly
  // instead of appearing connected but dead.
  std::move(callback).Run(/*bound=*/rule_based_engine_ != nullptr);
}

void RuleBasedEngineConnectionFactory::Unused(
    mojo::PendingAssociatedReceiver<mojom::JpUnused> unused,
