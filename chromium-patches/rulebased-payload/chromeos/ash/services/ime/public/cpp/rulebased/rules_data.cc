...
const std::map<std::string, RawDataEntry>& GetRawData() {
  // NOTE(colorburst fork): wrapped in base::NoDestructor; a plain static
  // std::map trips -Wexit-time-destructors, which is -Werror at r153.
  static const base::NoDestructor<std::map<std::string, RawDataEntry>>
      kRawData({
...
bool RulesData::IsIdSupported(const std::string& id) {
  // NOTE(colorburst fork): base::Contains was removed from //base after M113.
  return GetRawData().contains(id);
}
...
bool RulesData::PredictTransform(const std::string& str, int transat) const {
  ...
  // NOTE(colorburst fork): upstream iterates |str.length()| times here while
  // indexing into |s|, which is one byte longer whenever the transat delimiter
  // was inserted; the shortest suffix it considers is then two bytes and the
  // final single character is never tried. |s.length()| is the obviously
  // intended bound and is a no-op when transat <= 0 (s == str).
  // Measured on device (VIETNAMESE-IME.md 4.3): both variants produce
  // identical committed text for 21 Telex sequences, because an early "false"
  // here only commits the composition sooner -- the following keys then
  // transform in a fresh context and reach the same characters. Kept as a
  // correctness fix, not a behaviour fix.
  size_t len = s.length();
